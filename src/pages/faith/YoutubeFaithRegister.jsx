...
import {
  useDeleteYoutubeFaith,
  useSaveYoutubeFaith,
} from '../../api/youtubeFaithApi';
import { Button, Divider, Form, Icon, Segment } from 'semantic-ui-react';
import MSelect from '../../components/MSelect';

export default function YoutubeFaithRegister(props) {
  ...
  const { mutateSaveYoutubeFaith } = useSaveYoutubeFaith();
  const { mutateDeleteYoutubeFaith } = useDeleteYoutubeFaith();
  ...
  const onSubmit = (data) => {
    ...
    mutateSaveYoutubeFaith(reqData, {
      onSuccess: () => {
        alert('저장작업을 성공하였습니다.');
        reset();
        setFormData({
  ...
  const onDelete = () => {
    ...
    if (cfm) {
      ...
      mutateDeleteYoutubeFaith(reqData, {
        onSuccess: () => {
          alert('삭제작업을 성공하였습니다.');
          reset();
          setFormData({
