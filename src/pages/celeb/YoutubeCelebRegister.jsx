...
import {
  useDeleteYoutubeCeleb,
  useSaveYoutubeCeleb,
} from '../../api/youtubeCelebApi';
import { Button, Divider, Form, Icon, Segment } from 'semantic-ui-react';
import MSelect from '../../components/MSelect';

export default function YoutubeCelebRegister(props) {
  ...
  const { mutateSaveYoutubeCeleb } = useSaveYoutubeCeleb();
  const { mutateDeleteYoutubeCeleb } = useDeleteYoutubeCeleb();
  ...
  const onSubmit = (data) => {
    ...
    mutateSaveYoutubeCeleb(reqData, {
      onSuccess: () => {
        alert('저장작업을 성공하였습니다.');
        reset();
        setFormData({
  ...
  const onDelete = () => {
    ...
    if (cfm) {
      ...
      mutateDeleteYoutubeCeleb(reqData, {
        onSuccess: () => {
          alert('삭제작업을 성공하였습니다.');
          reset();
          setFormData({
