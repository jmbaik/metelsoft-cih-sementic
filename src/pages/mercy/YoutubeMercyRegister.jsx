...
import {
  useDeleteYoutubeMercy,
  useSaveYoutubeMercy,
} from '../../api/youtubeMercyApi';
import { Button, Divider, Form, Icon, Segment } from 'semantic-ui-react';
import MSelect from '../../components/MSelect';

export default function YoutubeMercyRegister(props) {
  ...
  const { mutateSaveYoutubeMercy } = useSaveYoutubeMercy();
  const { mutateDeleteYoutubeMercy } = useDeleteYoutubeMercy();
  ...
  const onSubmit = (data) => {
    ...
    mutateSaveYoutubeMercy(reqData, {
      onSuccess: () => {
        alert('저장작업을 성공하였습니다.');
        reset();
        setFormData({
  ...
  const onDelete = () => {
    ...
    if (cfm) {
      ...
      mutateDeleteYoutubeMercy(reqData, {
        onSuccess: () => {
          alert('삭제작업을 성공하였습니다.');
          reset();
          setFormData({
